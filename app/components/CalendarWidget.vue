...
const currentMonthLabel = computed(() => {
    const d = new Date(placeholder.value.year, placeholder.value.month - 1, 1)
    ...
    <UCard>
        <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-2">
                <UButton
icon="i-heroicons-chevron-left-20-solid" color="neutral" variant="ghost" size="sm"
                    @click="prevMonth" />
                <h3 class="text-base font-semibold leading-6 text-gray-900 dark:text-white capitalize">
                    {{ currentMonthLabel }}
                </h3>
                <UButton
icon="i-heroicons-chevron-right-20-solid" color="neutral" variant="ghost" size="sm"
                    @click="nextMonth" />
                <UButton size="xs" color="neutral" variant="ghost" class="ml-2" @click="goToToday">Avui</UButton>
            </div>
...
v-else :loading="externalCalendar.isLoading" icon="i-logos-google-icon" variant="soft"
        ...
        <div class="flex justify-center flex-col items-center">
            <UCalendar
v-model="date" v-model:placeholder="placeholder" locale="ca-ES" :fixed-weeks="false"
                :ui="{ header: 'hidden' }">
                <template #day="{ day }">
                    <div
