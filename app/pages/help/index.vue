...
<template>
  <div class="space-y-8 pb-10">
    <UBreadcrumb
:items="[
      { label: $t('components.header.home'), to: '/' },
      { label: $t('components.header.help'), to: '/help' }
    ]" />

    <!-- Header -->
    <div class="border-b border-gray-200 dark:border-gray-800 pb-6">
      <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">{{ $t('help.index.title') }}</h1>
      <p class="text-gray-500 dark:text-gray-400 mt-2">
        {{ $t('help.index.description') }}
      </p>
    </div>
    ...
    <section>
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-xl font-semibold text-gray-900 dark:text-white">{{ $t('help.index.topics_title') }}</h2>
        <UInput
v-model="searchQuery" icon="i-heroicons-magnifying-glass"
          :placeholder="$t('help.index.search_placeholder')" class="w-full max-w-xs" />
      </div>
      ...
      <div v-else class="text-center py-8">
        <p class="text-gray-500">{{ $t('help.index.no_results') }}</p>
        <UButton color="neutral" variant="ghost" class="mt-2" @click="searchQuery = ''">
          {{ $t('help.index.clear_filter') }}
        </UButton>
      </div>
    </section>
    ...
    <section class="border-t border-gray-200 dark:border-gray-800 pt-6">
      <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ $t('help.index.legal_section') }}</h2>
