...
const handleBackup = async () => {
    try {
        ...
        console.error(e)
        toast.add({ title: 'Error generant la còpia', color: 'error' })
    }
}
...
const handlePdf = async () => {
    try {
        ...
        saveAs(blob, `dietator-wrapped-${selectedYear.value}.pdf`)
...
<template>
    <div class="min-h-screen bg-gray-900 text-white -mx-4 -my-6 p-6 sm:p-8 font-sans transition-colors duration-500">
        ...
        <div class="fixed inset-0 z-0 overflow-hidden pointer-events-none">
            <div class="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] bg-purple-600/30 rounded-full blur-[120px] mix-blend-screen animate-pulse" />
            <div class="absolute bottom-[-20%] right-[-10%] w-[500px] h-[500px] bg-yellow-500/20 rounded-full blur-[100px] mix-blend-screen" />
        </div>
        ...
        <div class="relative z-10 max-w-4xl mx-auto space-y-12">
            ...
            <header class="flex flex-col sm:flex-row justify-between items-center gap-4">
                <div class="text-center sm:text-left">
                    <h1 class="text-5xl md:text-7xl font-black bg-gradient-to-r from-yellow-400 to-purple-400 bg-clip-text text-transparent">
                        El teu {{ selectedYear }}
                    </h1>
                    <p class="text-xl text-gray-400 mt-2">Resum de l'any a Dietator</p>
                </div>
                
                <div class="flex items-center gap-3 bg-gray-800/50 p-2 rounded-xl backdrop-blur-sm border border-gray-700/50">
                    <USelect 
            ...
            <section class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div class="bg-gray-800/40 p-6 rounded-3xl border border-gray-700/50 backdrop-blur-md hover:scale-105 transition-transform duration-300 delay-75">
                    ...
                    <div class="text-2xl lg:text-4xl font-black text-yellow-400">{{ formatNumber(stats.totalHours) }}<span class="text-lg text-gray-500 ml-1">h</span></div>
                    <div class="text-xs text-yellow-500/80 mt-1 font-medium">
                        ~{{ formatNumber(stats.weeklyAverageHours) }} h/setm · {{ formatNumber(stats.avgHoursPerService) }} h/serv
                    </div>
                </div>
                <div class="bg-gray-800/40 p-6 rounded-3xl border border-gray-700/50 backdrop-blur-md hover:scale-105 transition-transform duration-300 delay-100">
                    ...
                    <div class="text-xs text-purple-500/80 mt-1 font-medium">~{{ formatNumber(stats.weeklyAverageServices) }} serveis/setmana</div>
                </div>
                <div class="col-span-2 sm:col-span-1 bg-gray-800/40 p-6 rounded-3xl border border-gray-700/50 backdrop-blur-md hover:scale-105 transition-transform duration-300 delay-150">
                    <div class="text-gray-400 text-sm font-medium uppercase tracking-wider mb-2">Kilòmetres</div>
                    <div class="flex items-baseline gap-1 flex-wrap">
                        <span class="text-2xl lg:text-4xl font-black text-blue-400 break-words">{{ formatNumber(stats.totalKm) }}</span>
                        <span class="text-lg text-gray-500">km</span>
                    </div>
                    <div class="text-xs text-blue-500/80 mt-1 font-medium">
                        ~{{ formatNumber(stats.weeklyAverageKm) }} km/setm · {{ formatNumber(stats.avgKmPerService) }} km/serv
                    </div>
                </div>
            </section>
            ...
            <section class="bg-gray-800/30 p-4 sm:p-8 rounded-3xl border border-gray-700/50 backdrop-blur-sm">
                ...
                <div class="h-64 sm:h-80 flex justify-between gap-1 md:gap-4 items-end">
                    ...
                    <div v-for="(m, i) in stats.months" :key="i" class="relative flex-1 flex flex-col items-center justify-end h-full min-w-0 group">
                        <!-- Tooltip -->
                        <div class="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-all duration-200 transform translate-y-2 group-hover:translate-y-0 bg-gray-900/95 backdrop-blur border border-gray-700 p-3 rounded-xl shadow-2xl pointer-events-none z-30 w-max max-w-[200px] sm:max-w-none">
                            <div class="font-bold text-gray-100 mb-1 text-center border-b border-gray-800 pb-1">{{ getMonthName(i) }}</div>
                            <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-xs mt-1">
                                <span class="text-gray-400 text-left">Ingressos</span>
                                <span class="font-mono text-emerald-400 text-right">{{ formatCurrency(m.income) }}</span>
                        ...
                        <!-- Label -->
                        <div class="mt-3 text-[10px] sm:text-xs text-gray-400 font-medium uppercase truncate w-full text-center tracking-wider">{{ getMonthName(i).substring(0, 3) }}</div>
                    </div>
                </div>
            </section>
            ...
            <section class="grid md:grid-cols-2 gap-8">
                ...
                <div class="bg-gray-800/30 p-8 rounded-3xl border border-gray-700/50 backdrop-blur-sm">
                    ...
                    <div class="space-y-6">
                        <div v-for="comp in comparisons" :key="comp.label" class="space-y-2">
                            ...
                            <div v-if="comp.completed" class="text-xs text-green-400 font-bold text-right flex justify-end items-center gap-1">
                                <span>Completat! 🚀</span>
                                <span v-if="comp.timesCompleted > 1" class="bg-green-500/20 px-1.5 py-0.5 rounded text-green-300">x{{ comp.timesCompleted }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    <div class="bg-gradient-to-br from-yellow-900/40 to-orange-900/40 p-8 rounded-3xl border border-yellow-700/30">
                    ...
                    <div v-if="stats.mostActiveDay.date" class="bg-gradient-to-br from-pink-900/40 to-purple-900/40 p-8 rounded-3xl border border-pink-700/30">
                        <h4 class="text-pink-300 text-lg font-bold mb-2">Dia més actiu 🔥</h4>
                        <div class="text-3xl font-black text-white mb-1">
                            {{ new Date(stats.mostActiveDay.date).toLocaleDateString('ca-ES', { day: 'numeric', month: 'long' }) }}
                        </div>
                        <div class="text-pink-200/80">
                            Vas treballar <span class="font-bold text-white">{{ formatNumber(stats.mostActiveDay.hours) }} hores</span> aquest dia.
                        </div>
                    </div>

                    <div v-if="stats.mostKmDay.date" class="bg-gradient-to-br from-blue-900/40 to-cyan-900/40 p-8 rounded-3xl border border-blue-700/30">
                        <h4 class="text-blue-300 text-lg font-bold mb-2">Dia més viatger 🚗</h4>
                        <div class="text-3xl font-black text-white mb-1">
                            {{ new Date(stats.mostKmDay.date).toLocaleDateString('ca-ES', { day: 'numeric', month: 'long' }) }}
                        </div>
